...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        fy= K[1,1]
        eps = 1e-4
        J_proj[:,0,0] = fx/cam_points[:,2]
        J_proj[:,0,1] = 0
        J_proj[:,0,2] = -fx*cam_points[:,0]/torch.pow(cam_points[:,2]+eps,2)
        J_proj[:,1,0] = 0
        J_proj[:,1,1] = fy/cam_points[:,2]
        J_proj[:,1,2] = -fy*cam_points[:,1]/torch.pow(cam_points[:,2]+eps,2)

        # Transform covariance to camera space
        ### FILL: Aplly world to camera rotation to the 3d covariance matrix
        ### covs_cam = ...  # (N, 3, 3)
        R = R.view(1,3,3)
        covs_cam = torch.matmul(R,torch.matmul(covs3d, R.permute(0, 2, 1)))
        # covs_cam = torch.einsum('ij, n',torch.einsum('n'))
        # Project to 2D
