...
def train_one_epoch(generator, discriminator, dataloader, optimizer_g, optimizer_d, l1loss, device, epoch, num_epochs):
    ...
    for i, (image_rgb, image_semantic) in enumerate(dataloader):
        ...
        loss_d = lambda2 * F.binary_cross_entropy(real_predict, real_label) + lambda2 * F.binary_cross_entropy(fake_predict, fake_label)
        loss_d.backward()
        optimizer_d.step()
        ...
        if epoch % 50 == 0 and i == 0:
            save_images(image_rgb, image_semantic, G_outputs, 'train_results/gan/city', epoch)
...
def validate(generator, discriminator, dataloader, l1loss, device, epoch, num_epochs):
    ...
    with torch.no_grad():
        for i, (image_rgb, image_semantic) in enumerate(dataloader):
            ...
            if epoch % 50 == 0 and i == 0:
                save_images(image_rgb, image_semantic, G_outputs, 'val_results/gan/city', epoch)
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 50 == 0:
            os.makedirs('checkpoints/gan/city', exist_ok=True)
            torch.save(generator.state_dict(), f'checkpoints/gan/city/pix2pix_generator_epoch_{epoch + 1}.pth')
            torch.save(discriminator.state_dict(), f'checkpoints/gan/city/pix2pix_discriminator_epoch_{epoch + 1}.pth')
