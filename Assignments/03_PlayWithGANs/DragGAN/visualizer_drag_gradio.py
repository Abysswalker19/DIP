...
with gr.Blocks() as app:
    ...
    with gr.Row():
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
                        gr.Markdown(value='App', show_label=False)
                    with gr.Column(scale=4, min_width=10):
                        with gr.Row():
                            with gr.Column(scale=1, min_width=10):
                                smile_points = gr.Button('Smile')
                            with gr.Column(scale=1, min_width=10):
                                thin_face_points = gr.Button('Thin face')
                        with gr.Row():
                            with gr.Column(scale=1, min_width=10):
                                big_eyes_points = gr.Button("Big eyes")
                            with gr.Column(scale=1, min_width=10):
                                close_eyes_points = gr.Button("Close eyes")
    ...
    smile_points.click(on_click_smile, 
            inputs=[global_state],
            outputs=[global_state, form_image])

    def on_click_thin_face(global_state):
        if global_state['editing_state'] != 'add_points':
            print(f'In {global_state["editing_state"]} state. '
                  'Do not add points.')
            return global_state, global_state['images']['image_show']
        ...
        points[point_idx + 1] = {'start': [int(preds[0,0]), int(preds[0,1])],\
         'target': [int(preds[0,0])+30, int(preds[0,1])]}
        points[point_idx + 2] = {'start': [int(preds[3,0]), int(preds[3,1])],\
         'target': [int(preds[3,0])+20, int(preds[3,1])]}
        points[point_idx + 3] = {'start': [int(preds[13,0]), int(preds[13,1])],\
         'target': [int(preds[13,0])-20, int(preds[13,1])]}
        points[point_idx + 4] = {'start': [int(preds[16,0]), int(preds[16,1])],\
         'target': [int(preds[16,0])-30, int(preds[16,1])]}
    ...
    thin_face_points.click(on_click_thin_face, 
      inputs=[global_state],
      outputs=[global_state, form_image])
    ...
    big_eyes_points.click(on_click_big_eyes, 
      inputs=[global_state],
      outputs=[global_state, form_image]
    )
    ...
    def on_click_close_eyes(global_state):
        ...
        points[point_idx + 1] = {'start': [int(preds[37,0]), int(preds[37,1])],\
         'target': [int(preds[37,0]), (int(preds[37,1])+int(preds[41,1]))/2.0]}
        points[point_idx + 2] = {'start': [int(preds[41,0]), int(preds[41,1])],\
         'target': [int(preds[41,0]), (int(preds[37,1])+int(preds[41,1]))/2.0]}
        points[point_idx + 3] = {'start': [int(preds[38,0]), int(preds[38,1])],\
         'target': [int(preds[38,0]), (int(preds[38,1])+int(preds[40,1]))/2.0]}
        points[point_idx + 4] = {'start': [int(preds[40,0]), int(preds[40,1])],\
         'target': [int(preds[40,0]), (int(preds[38,1])+int(preds[40,1]))/2.0]}

        points[point_idx + 5] = {'start': [int(preds[43,0]), int(preds[43,1])],\
         'target': [int(preds[43,0]), (int(preds[43,1])+int(preds[47,1]))/2.0]}
        points[point_idx + 6] = {'start': [int(preds[47,0]), int(preds[47,1])],\
         'target': [int(preds[47,0]), (int(preds[43,1])+int(preds[47,1]))/2.0]}
        points[point_idx + 7] = {'start': [int(preds[44,0]), int(preds[44,1])],\
         'target': [int(preds[44,0]), (int(preds[46,1])+int(preds[44,1]))/2.0]}
        points[point_idx + 8] = {'start': [int(preds[46,0]), int(preds[46,1])],\
         'target': [int(preds[46,0]), (int(preds[46,1])+int(preds[44,1]))/2.0]}
